...
export const getQAResults = async (query, from) => {
  const source = await fetch(
    `${process.env.ORIGIN_URL}/api/index.php/search/hoidap`,
    {
      method: "POST",
      headers: {
...
export const getPostResults = async (query, from) => {
  const source = await fetch(
    `${process.env.ORIGIN_URL}/api/index.php/search/lecttr`,
    {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
