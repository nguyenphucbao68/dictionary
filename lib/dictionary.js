...
function findEnglishDefinitionLexico(word, language, callback) {
	...
	return giveBody(URI, async (err, body) => {
						...
						$(element)
							...
							.each(function (j, element) {
								...
								$(item)
									...
									.each((j, element) => {
										...
										if (synonyms.length > 0)
											subSenseObj.synonyms = synonyms;

										if (examples.length > 0)
											subSenseObj.examples = examples;
								...
								if (subSenses.length > 0)
									newDefinition.subSenses = subSenses;
				...
				$(grambs[j])
					...
					.each(function (j, element) {
						var meaningArray = [],
							...
							$(item).find(' > .examples .english-ex .ex')
							...
							if(example.length > 0)
								objEmptySense.example = example;
							if(synonymsText.length > 0)
								objEmptySense.example = synonymsText;
							if(examples.length > 0)
								objEmptySense.examples = examples;
						meaningArray.push(objEmptySense);
						entry.meaning.push({ name: partofspeech, data: meaningArray});
					})
			}
			...
			for (j = start; j < end; j++) {
				...
				if (!$(etymology[j]).find('.phrase').first()[0]) {
					var contentOriginal = $(etymology[j])
						...
						.text();
					contentOriginal = contentOriginal.replace(/(\s|)Compare with (.*?)\./g, '');
					entry.meaning.push({name : etymologyName, data: contentOriginal});
				} else {
				$(etymology[j])
					.find('.phrase')
					.each((l, element) => {
				...
				// etymologyName = 'other';
				entry.meaning.push({name: etymologyName, data: etymologyArray});
				}
			}
		...
		return { data: dictionary, relatedWords }
	});
}

async function findRelatedWords(word, limit = 8, host = "http://localhost/restful"){
	try {
		// console.log(`/api/index.php/search/${word}/${limit}`);
		const response = await fetch(`${process.env.ORIGIN_URL}/api/index.php/search/${word}/${limit}`);
		if(response) {
